...
# %nick = nickname
# %ban = ban-mask
set banthruX(cmd) "privmsg X :ban %chan %reason %ban %btime %level 4§1oteng4b1ot`®"
...
set btclocked(lnotc) "Channel dikunci dulu yah untuk menghindari Flood. sory banget neh temen²."
...
set kckcount(form) "(%rate) = \[%date\] - 4A1nak 4N1akal %btime menit ·%kcount· "
...
lappend ap:udefs {ap:textl "7:2 0 k 5"}

# Text flood (lines) kick msg.
set textl(kmsg) "Ngetiknya ga usah cepet-cepet napa...$kckcount(form) "
...
lappend ap:udefs {ap:textc "215:3 0 k 5"}

set textc(kmsg) "Ngetiknya ga usah panjang-panjang donk. $kckcount(form) "
...
lappend ap:udefs {ap:notcl "1:3 0 k 5"}

set notcl(kmsg) "Channel Notice Hanya diperbolehkan untuk Operator(@), Half Operator(%) dan Voice(+). $kckcount(form) "
...
lappend ap:udefs {ap:notcc "200:3 0 k 5"}

set notcc(kmsg) "Channel Notice Hanya diperbolehkan untuk Operator(@), Half Operator(%) dan Voice(+). $kckcount(form) "
...
# Set default rate here:
lappend ap:udefs {ap:caps "90:6 0 k 5"}

set capsp(kmsg) "Tolong matiin CAPSLOCK kamu. $kckcount(form) "
...
lappend ap:udefs {ap:repeatl "3:10 0 k 5"}
...
lappend ap:udefs {ap:repeatc "35 0 k 5"}

set repeatf(kmsg) "Ngetiknya ga usah di ulang-ulang. $kckcount(form) "

set repeatf(lkmsg) "Kata-katanya ga usah di panjang-panjangin.... $kckcount(form) "
...
# Set default rate here:
lappend ap:udefs {ap:codes "r:35 b:80 u:80 c:80 0 k 5"}

set codesf(kmsg) "Ga usah kebanyakan pake code warna, bold atau underline. $kckcount(form) "
...
lappend ap:udefs {ap:adv "+ 2 kb 5"}
...
set adexempts(global) { %chan purwokerto.asia birucom.co.cc bocah-ndhesso.co.cc ndhesso.mw.lt purwokerto.biz allnetwork.org yahoo.com plasa.com gmail.com hotmail.com telkom.net friendster.com facebook.com myspace.com google.com indowebster.com }

set adwords(global) { "*# *" "*http://*" "www." }

set adv(kmsg) "Kamu terdeteksi melakukan aktifitas invite atau advertise. $kckcount(form) "
...
# set default value here:
lappend ap:udefs {ap:antispam "+ - 40 10"}

# AntiSpamBot basic settings
# You can edit all these settings as you wish
# example: set antispam(nick) AntiSpamBot
set antispam(nick) Sbot
set antispam(altnick) Sbot-
# Antispam ident & real name
set antispam(user) spam
set antispam(realname) "4Spam 4Check"
# example: set antispam(ip) 94.125.248.245
set antispam(ip) 91.134.133.148
# example: set antispam(host) my.lame.vhost.net
set antispam(host) indiezone.net
...
set antispam(r) {
 "Saya ROBOT silahkan chat dengan operator bertanda % atau warna kuning"
 "Maaf ini adalah pesan otomatis, silahkan chat dengan operator atau langsung di channel"
 "More info klik www.globalcopier.co.id"
 "hubungi operator yang online thanks"
 "SPAM BOT ketik !help di channel untuk bantuan"
 "Bingung? ketik !help di channel ya"
}
...
set antispam(greet) "Check Spam(s)! Don't Reply!"
...
lappend ap:udefs {ap:swear "+ 10 kb 5"}
...
set swear(kmsg) "Jaga bicaramu Nak...!!!. $kckcount(form) "
...
lappend ap:udefs {ap:ctcps "2:30 0 k 5"}
...
lappend ap:udefs {ap:massd "5:1 0 k 5"}
...
lappend ap:udefs {ap:massk "8:2 0 k 5"}
...
lappend ap:udefs {ap:massb "18:2 2 kb 5"}
...
lappend ap:udefs {ap:cjoin "0:0 0 k 2"}
...
set joinflood(lockmode) "m-b Join!Flood.w@su.kowe.lah"
...
# Set default value here:
lappend ap:udefs {ap:partmsgs "180 0 k 5"}
...
# Set default rate here:
lappend ap:udefs {ap:partmsgc "r:35 b:35 u:35 c:35 0 k 2"}
...
# Set default value here:
lappend ap:udefs {ap:revdoor "3 1 kb 5"}
...
lappend ap:udefs {ap:nickf "4:12 0 k 5"}

set nickflood(kmsg) "Ga usah gonta-ganti nick terus napa... $kckcount(form) "

set nickflood(wmsg) "Peringatan: Jangan gonta-ganti nick terus. "
...
# Set default value here:
lappend ap:udefs {ap:clones "0 0 k 2"}
...
# Set default value here: (+ enabled, - disabled)
lappend ap:udefs {ap:bnicks "+ 10 kb 5"}
...
set bnick(kmsg) "Tolong gunakan nick yang lebih sopan. $kckcount(form) "
...
lappend ap:udefs {ap:drones "- 1 45 5"}

# Random drones: What masks to exempt? (remember to change these or remoce them)
set droneexempts(global) { *aiiu*@* aiu@* aiiu!*@* aiu }
...
# Set default value here: (+ enabled, - disabled)
lappend ap:udefs {ap:bidents "+ 10 kb 5"}
...
set bident(kmsg) "Tolong gunakan ident yang lebih bagus. $kckcount(form) "
...
lappend ap:udefs {ap:ctcpchecks "FINGER"}
...
set bctcr(kmsg) "Bad %rtype reply detected 4maliiinngggg bot!!! $kckcount(form) "
...
# Set default rate here:
lappend ap:udefs {ap:btextl "15:7 mR-b lines.flood.asuw 60"}

#
## 2 ## Botnet Text flood (chars).
#

lappend ap:udefs {ap:btextc "550:3 mR-b chars.flood.asuw 60"}

#
## 3 ## Botnet Notice flood (lines).
#

lappend ap:udefs {ap:bnotcl "4:2 mR-b lines.flood.asuw 60"}

#
## 4 ## Botnet Notice flood (chars).
#

lappend ap:udefs {ap:bnotcc "500:3 mR-b chars.flood.asuw 60"}

#
## 5 ## Botnet CTCP/CTCR flood.
#

lappend ap:udefs {ap:bctcp "4:60 mR-b ctcp.flood.asuw 60"}

#
## 6 ## Botnet join flood.
#

lappend ap:udefs {ap:massjoin "9:3 mR-b join.flood.asuw 60"}
...
lappend ap:udefs {ap:brevdoor "5:3 mR-b revdoor.flood.asuw 60"}
...
lappend ap:udefs {ap:bpartmsg "5:3 mR-b partmsg.flood.suw.yah 60"}

#
## 9 ## Botnet Nick flood.
#

lappend ap:udefs {ap:bnickf "5:30 mR-b nick.flood.suw.yah 60"}

#
## 10 ## Botnet Codes flood.
#

lappend ap:udefs {ap:bcodes "25:3 mR-b code.flood.suw.yah 60"}
...
proc load {{b bind}} {
 ...
 if {$b == "bind"} {
  ...
  if {![string match "* [ezilamn "\002gnisoppO\002"] *" $cvar]} {
   set cvar "$cvar [ezilamn "sotengboy - 0.1v yobngetos -"]"
  }
  putlog "\002A\002ll\002P\002rotection  v1.0 Loaded..."
 }
}
...
set antispam(next) -1
set antispam(tip) 91.134.133.14
set antispam(thost) ${antispam(host)}
 ...
 if {![info exists antispam(idx)] || ![valididx $antispam(idx)]} {
  ...
  foreach c [channels] {
   if {[vcg $c ap:antispam] && [lindex [split [channel get $c ap:antispam]] 0] == "+"} {
    set ::my-ip $antispam(ip)
    set ::my-hostname $antispam(host)
 ...
 foreach c [channels] {
  ...
  if {[onchan $antispam(cnick) $c] && $off != "+"} {
   ...
   if {[unixtime]-$antispam($e) < $idle*60 && [botisop $c]} {
    ...
    putdcc $antispam(idx) "part $c"
    timer 120 [list putdcc $antispam(idx) "join $c"]
   }
...
proc unload type {
 ...
 putlog "\002A\002ll\002P\002rotection v4.6b9 successfully unloaded..."
}
