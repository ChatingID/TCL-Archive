#   film.tcl Update film terbaru dari Layarkaca21
#                                                                     
#        Usage: !film <feed name> [news index #] - from channel       
...
# RSS Feed
set feeds(Lk21:#ndhesso) {30 http://lk21.tv/feed/ }
set feeds(Lk21:#purwokerto) {30 http://lk21.tv/feed/ }
set feeds(Lk21:##) {30 http://lk21.tv/feed/ }
...
variable version "Berita TCL V2.1"
...
bind pub  - !film [namespace current]::news
bind time - *     [namespace current]::timer

putlog "$version by Chimp loaded"
	...
	foreach {foo item} [regexp -all -inline {(?i)<item.*?>(.*?)</item>} $data] {
		...
		if {[lsearch -exact $hash($chanfeed) [md5 $title]] == -1 && [botonchan $chan]} {
			...
			if {$count < $maxnew} {
				...
				puthelp "PRIVMSG $chan :Update Film: \0034$title\003 -$descr ($link)"
				incr count
			} {
				lappend indices $idx
			}
...
proc news {nick uhost hand chan text} {
	...
	if {$num == ""} {
		...
		puthelp "notice $nick :Upload Terbaru dari: \002$title\002"
		foreach item $news($feed:$chan) {
			puthelp "notice $nick :($idx) [lindex $item 0] $link"
			incr idx
